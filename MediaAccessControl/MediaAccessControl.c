...
static int Receiver(struct sk_buff*skb,struct net_device*dev){
    ...
    return ntohs(*(u16*)(dataLink+12))==2048?
        GetRFC791()->Receiver(networkLayer,&sourceAddress,&destinationAddress,&sourcePort,&destinationPort,&transportLayer)?
            ContinueReceiver2048(skb,dev,router,dataLink,networkLayer,transportLayer,sourceAddress,destinationAddress,sourcePort,destinationPort):0:
        GetRFC8200()->Receiver(networkLayer,&sourceAddress,&destinationAddress,&sourcePort,&destinationPort,&transportLayer)?
            ContinueReceiver34527(skb,dev,router,dataLink,networkLayer,transportLayer,sourceAddress,destinationAddress,sourcePort,destinationPort):0;
}

SystemSetup(MediaAccessControl, NULL, NULL,Bind(Receiver));